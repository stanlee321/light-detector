...
import logging
import datetime
datetime = datetime.datetime.now().strftime('%Y-%m-%d_%H_%M_%S')
LOG_PATH = 'LOGGIN_semaforo_{}.log'.format(datetime)
...
def main(video):
    ...
    while True:
        ...
        logger.info('np.where(puck!=0)[0]' + str((np.where(puck!=0)[0])))
        logger.info('np.where(puck!=0)[0]' + str((np.where(puck!=0)[1])))
...
def track(video):
    ...
    archivoParametrosACargar = '/trafficFlow/prototipo/installationFiles/' + 'datos.npy'

    parametrosInstalacion = np.load(home + archivoParametrosACargar)
    indicesSemaforo = parametrosInstalacion[0]
    ...
    while (1):
        ...
        logger.info('lower blue'+ str( (h,s,v)))
